...
$("#file_base").off("change.mainDrop").on("change.mainDrop", function (e) {
  ...
  $("#formLlamado_base").on("submit", function(e){
    ...
    Swal.fire({
      ...
      text: "Se registrará la instrucción de la cancelación.",
      icon: "question",
 ...
 * Key: "cancelacion_inst_corto_plazo_papel_comercial"
 */
function aprobacion_inst_corto_plazo(){
  try {
    const KEY = "cancelacion_inst_corto_plazo_papel_comercial";
 ...
 * Lee de localStorage clave "cancelacion_inst_corto_plazo_papel_comercial".
 * Reglas:
...
function __getListaAprobacion(KEY = "cancelacion_inst_corto_plazo_papel_comercial"){
  let lista;
  try { lista = JSON.parse(localStorage.getItem(KEY) || "[]"); }
  catch { lista = []; }
