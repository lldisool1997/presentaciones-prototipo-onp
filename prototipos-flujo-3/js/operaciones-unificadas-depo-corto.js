 ...
 * Key: "aprobacion_inst_corto_plazo_depo_corto"
 */
...
function __buildCartaUrl({ panelId, operacionUrl, transferenciaUrl, paramName }) {
  ...
  return `${baseUrl}${sep}${encodeURIComponent(key)}=${encodeURIComponent(panelId)}&area=Tesoreria&storage_key_carta=aprobacion_inst_corto_plazo_depo_corto`;
}
 ...
 * Lee de localStorage clave "aprobacion_inst_corto_plazo_depo_corto".
 * Reglas:
...
function __getListaAprobacion(KEY = "aprobacion_inst_corto_plazo_depo_corto"){
  let lista;
  try { lista = JSON.parse(localStorage.getItem(KEY) || "[]"); }
  catch { lista = []; }
