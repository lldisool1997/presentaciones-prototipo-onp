...
function build_aprobacion_snapshot(){
  ...
  const comision = __parseMontoToNumber(($("#comision").val() || "").trim());
    ...
    const KEY = "aprobacion_inst_cta_remunerada";
 ...
 * Key: "aprobacion_inst_cta_remunerada"
 */
 ...
 * Lee de localStorage clave "aprobacion_inst_cta_remunerada".
 * Reglas:
...
function __getListaAprobacion(KEY = "aprobacion_inst_cta_remunerada"){
  let lista;
  try { lista = JSON.parse(localStorage.getItem(KEY) || "[]"); }
  catch { lista = []; }
...
function openInteresModal({ banco='INTERBANK — 200-3067561380', tea=4.60, tna=4.51, start, end, saldoInicial=1_000_000.00 }={}){
  ...
  // abrir / cerrar
  $modal.removeClass('hidden').addClass('flex');
  $modal.off('click.close').on('click.close', function(e){
    if ($(e.target).is('#interesModal, .btnClose')) {
      $modal.addClass('hidden').removeClass('flex');
    }
  });
}
